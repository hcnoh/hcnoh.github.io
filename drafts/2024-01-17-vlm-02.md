...
ViT는 CNN에 비해서 훨씬 적은 이미지 특화된 귀납적 편향을 지니고 있다고 한다.

ViT는 CNN에 비해서 적은 귀납적 편향을 가지고 있는 대신 학습을 위해서는 더 많은 데이터를 필요로 하며 이미지 내에서 지엽적인 정보를 추출하는 데에 조금 더 불리하다.
...
Self-Attention은 ViT로 하여금 가장 초반의 계층에서도 전체 이미지 정보를 통합할 수 있도록 한다. 잘 학습된 CNN의 특징 맵(Feature Map)을 살펴보면 초반부 계층에서는 특징 맵이 특정 도형 모양 등의 매우 일반적인 특징을 지녔다면 후반부 계층으로 갈 수록 점점 더 구체적인 모양으로 변화하는 것을 확인할 수 있다. 이는 초반부 계층에서는 아주 지엽적인 정보들을 추출하고 후반부 계층으로 갈 수록 점점 더 전체적인 정보들을 추출하도록 모델이 학습되어 진다는 점을 나타낸다고 볼 수 있다.
