...
초기 연구들은 비지도 파인 튜닝 접근을 위하여 사전 학습된 단어(토큰) 임베딩을 활용하는 정도였다. 반면 최근 연구들은 비지도 방식을 통해서 문장, 또는 문서에 대한 임베딩 모델에 대해 사전 학습을 시도하고 있다. 이러한 접근의 좋은 점은 매우 적은 매개변수만이 갱신이 되는 정도라서 파인 튜닝 과정이 매우 짧다는 점이다.
