...
## BERT의 사전 학습
기존의 언어 모델들과 다르게 BERT는 정방향 또는 역방향의 사전 학습을 수행하지 않는다. 대신 두 가지의 비지도 태스크를 통해서 사전 학습이 진행된다. 두 가지 태스크는 각각 `마스크 언어 모델`(Masked LM, MLM)과 `다음 문장 예측`(Next Sentence Prediction, NSP)이다.
