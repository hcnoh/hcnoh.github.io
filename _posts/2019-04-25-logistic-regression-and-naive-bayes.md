...
& = \arg \max_{\boldsymbol{\theta}} \prod_{(\mathbf{x}, y) \in (\mathcal{X}, \mathcal{Y})} \sum_{(\mathbf{x}, y) \in (\mathcal{X}, \mathcal{Y})} \left[ \log \pi_y + \sum_{i\in \{1,2,\cdots, d \}} \log q_{(y, \boldsymbol{\theta})}^{(i)}(x^{(i)})
\end{align*} \right]
$$
