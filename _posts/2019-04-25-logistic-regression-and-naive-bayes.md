...
& = \arg \max_{\boldsymbol{\theta}} \sum_{\mathbf{x}\sim P_D(\mathbf{x})} N_D(\mathbf{x})P_D(Y=1 \vert \mathbf{x})\log(\mu(\mathbf{x})) + N_D(\mathbf{x})P_D(Y=0 \vert \mathbf{x})\log(1-\mu(\mathbf{x}))
\end{align*}
$$
