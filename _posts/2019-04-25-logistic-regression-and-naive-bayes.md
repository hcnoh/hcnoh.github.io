...
& = \frac{1}{1 + \exp \left( \log \left[ \frac{\prod_{i\in \{1,2,\cdots, d \}} q_{(0, \boldsymbol{\theta})}^{(i)}(x^{(i)})\pi_0}{\prod_{i\in \{1,2,\cdots, d \}} q_{(1, \boldsymbol{\theta})}^{(i)}(x^{(i)})\pi_1} \right] \right) }
\end{align*}
$$
...
& = \arg \max_{\boldsymbol{\theta}} \sum_{(\mathbf{x}, y) \in (\mathcal{X}, \mathcal{Y})} \left[ \log \pi_y + \sum_{i\in \{1,2,\cdots, d \}} \log q_{(y, \boldsymbol{\theta})}^{(i)}(x^{(i)}) \right]
\end{align*}
$$
