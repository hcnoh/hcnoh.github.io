...
![](assets/img/2023-02-25-nlp-03/2023-03-02-21-39-43.png){: width="80%" height="80%"}

위 그림은 Transformer의 계층별 복잡도를 설명하기 위한 그림이다. 행렬 $$\mathbf{Q}$$와 $$\mathbf{V}$$를 곱하기 위해서 필요한 계산량은 결과 행렬의 크기인 $$n^2$$에다가 각 요소를 구하기 위해 필요한 계산량 $$d$$를 곱하여 $$n^2 \cdot d$$가 된다. 그 다음으로 $$\text{Softmax} \left( \frac{\mathbf{QV}^\text{T}}{\sqrt{d}} \right)$$와 $$\mathbf{V}$$를 곱하기 위해서는 결과 행렬의 크기인 $$n \cdot d$$와 더불어 각 성분을 계산하기 위해서 필요한 $$L$$을 곱하여 마찬가지로 $$n \cdot d$$가 된다. 따라서 Transformer의 계층별 복잡도는 $$n^2 \cdot d + n^2 \cdot d = O(n^2 \cdot d)$$가 된다.
