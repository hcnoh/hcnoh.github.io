...
- 지역 이름들을 포함하는 현재 함수의 스코프
- 전역 이름들을 포함하는 현재 모듈의 스코프
- 빌트인 이름들을 포함하는 최외곽의 스코프

만약 새로운 함수가 실행된다면 그 함수의 지역 이름들을 포함하는 새로운 스코프가 생겨서 중첩되게 된다. 또한 현재 변수의 이름이 참조하는 객체를 검색하는 경우에는 가장 내부의 지역 이름들을 포함한 스코프부터 검색해 나가며 그 다음에는 보다 바깥에 있는 지역 이름들을 포함한 스코프를, 그리고 마지막에는 전역 이름들을 포함한 모듈 스코프를 검색한 이후에 최종적으로 빌트인 이름들을 포함한 최외곽 스코프를 검색하게 된다.
