...
즉, 맨 처음 질문의 `if __name__ == '__main__':`을 해석하면 다음과 같다. `__name__`은 현재 모듈의 네임스페이스의 이름을 담고있는 내장 변수이다. 따라서 현재 모듈의 네임스페이스가 `__main__`에 해당한다면, 즉 커맨드 라인 상에서 실행한 모듈이라면 그 이하를 실행하라는 의미이다.
