...
## Tacotron의 트레이닝 및 인퍼런스
Tacotron은 Attention Mechanism을 포함한 Encoder-Decoder 구조이기 때문에 트레이닝 및 인퍼런스 방식은 일반 Encoder-Decoder 모델과 같다. 먼저 트레이닝의 경우에는 입력 텍스트 및 타겟 오디오를 전처리하여 Mel-Spectrogram과 Linear-Spectrogram을 준비해놔야 한다. 즉 정리하자면, 입력은 텍스트, 타겟은 Mel-Spectrogram 및 Linear-Spectrogram을 필요로 하게 된다.
