...
간단하게 정리하면 다음과 같다. 현재 타임 스텝의 디코더 출력 $$y_i$$를 생성하기 위해서는 인코더에 입력되는 문장 시퀀스 $$\mathbb{x}$$와 이전 타임 스텝들의 디코더 출력 시퀀스 $$\{ y_1, \cdots, y_{i-1}\}$$이 Conditioning되는 조건부 확률 $$p(y_i | y_1,\cdots, y_{i-1}, \mathbb{x})$$는 RNN Encoder-Decoder 모델에서 다음과 같이 모델링될 수 있다.

$$
\begin{align*}
p(y_i | y_1,\cdots, y_{i-1}, \mathbb{x}) & = g(s_{i-1}, c_i) \\
